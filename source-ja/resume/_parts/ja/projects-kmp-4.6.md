...
**プロジェクト説明**：独立開発のクロスプラットフォームショートビデオ+画像ギャラリー閲覧アプリで、Compose Multiplatformを採用して一度のコーディングで複数プラットフォームで動作する最新の宣言型UIを実現。製品形態は抖音/小紅書のショートビデオと画像コミュニティを参考にしており、スムーズな動画/画像のスワイプ閲覧、カテゴリ別表示、フルスクリーン没入型インタラクティブ体験を提供し、Android、iOS、JVMなどの複数プラットフォームをサポート。    
**機能モジュール**：
