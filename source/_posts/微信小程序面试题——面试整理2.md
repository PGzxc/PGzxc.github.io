...
computed监听会在onload生命周期钩子前执行，也会在onshow页面显示的钩子中会执行watch 
...
watch监听的函数名需要和被监听的data中的数据名一样，儿computed可以自己定义，且写完逻辑后需要return 值。当数据真正被修改的时候才会被执行，而computed在onLoad之前都会被执行
